...
## Code structure
This is a multi-module cross-platform Gradle project. The core is written in Java (plus some HTML/JS/CSS for a simple web frontend); the desktop GUI app is Kotlin with Compose Multiplatform. I usually edit it in Android Studio. It has the following modules:
...
- **shttps-desktop-common**: J2SE implementations of the platform interfaces from shttps-common (SQLite-backed database, file-based config, and so on). Shared by shttps-commandline and shttps-desktop.
- **shttps-desktop**: the desktop GUI application, built with Kotlin and Compose Multiplatform on top of shttps-desktop-common. Run it with `./gradlew :shttps-desktop:run`, or build a native installer with `./gradlew :shttps-desktop:packageMsi` (`packageDmg`/`packageDeb`/`packageRpm` on the matching OS) — output goes to the `builds` folder. It needs a Java 21 toolchain, which Gradle will download automatically if you don't have one. `packageRpm` additionally needs `rpmbuild` on the build machine (`rpm` on Debian/Ubuntu, `rpm-build` on Fedora); `packageDeb` needs `dpkg-deb` and `fakeroot`, which a Debian based system already has.
