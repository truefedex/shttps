...
async function onPageLoad() {
    ...
    try {
        databaseSchema = await api('GET', '/api/db/schema');
...
async function duplicateTable(tableName) {
    if (!confirm(`Are you sure you want to duplicate the table "${tableName}"?\n\nNote: This method will copy the data but not the constraints (primary keys, foreign keys, etc.).`)) return;
    document.getElementById('loader').style.display = 'block';
    const newTableName = `${tableName}_copy`;
    try {
        await runSql(`CREATE TABLE "${newTableName}" AS SELECT * FROM "${tableName}";`);
