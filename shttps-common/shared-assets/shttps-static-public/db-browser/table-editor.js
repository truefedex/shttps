...
function onPageLoad() {
    ...
    document.querySelector('.apply-table-name').addEventListener('click', async function() {
        ...
        try {
            await runSql(`ALTER TABLE "${originalName}" RENAME TO "${newName}";`);
...
async function loadTableStructure(tableName) {
    ...
    try {
        ...
        const data = await api('GET', `/api/db/schema?table=${encodeURIComponent(tableName)}`);
        if (data.columns) {
        ...
        alert('Error loading table structure: ' + error.message);
    } finally {
        document.getElementById('loader').style.display = 'none';
    }
}
...
async function renameColumn(tableName, oldName, newName, columnId) {
    // Show loader
    document.getElementById('loader').style.display = 'block';
    try {
        await runSql(`ALTER TABLE "${tableName}" RENAME COLUMN "${oldName}" TO "${newName}";`);
...
async function deleteColumn(tableName, columnName) {
    ...
    document.getElementById('loader').style.display = 'block';
    try {
        await runSql(`ALTER TABLE "${tableName}" DROP COLUMN "${columnName}";`);
    } catch (error) {
